...
def render_player_search(data_provider, filtered_data=None):
    ...
    use_date_filter = st.checkbox("Enable Date Filter", value=False, help="Filter matches by date range", key="app_search_date_filter")
    if use_date_filter:
        ...
        if start_date and end_date:
            ...
            filtered_player_count = len(player_data)
            st.success(f"✅ Date filter applied: {filtered_player_count} players found (was {original_player_count})")
            if st.checkbox("🔍 Show Date Filter Debug Info", value=False, key="app_search_debug"):
...
def render_player_comparison(data_provider, filtered_data=None):
    ...
    # === DATE FILTER SECTION ===
    st.subheader("📅 Date Filter")
    use_date_filter = st.checkbox("Enable Date Filter", value=False, help="Filter matches by date range", key="app_comparison_date_filter")
    if use_date_filter:
        col1, col2 = st.columns(2)
        with col1:
            start_date = st.date_input(
        ...
        if start_date and end_date:
            ...
            filtered_player_count = len(player_data)
            st.success(f"✅ Date filter applied: {filtered_player_count} players found (was {original_player_count})")
            if st.checkbox("🔍 Show Date Filter Debug Info", value=False, key="app_comparison_debug"):
                render_debug_interface(player_data, original_player_count, filtered_player_count, start_date, end_date)
            if not player_data:
                st.warning("No players found with matches in the selected date range.")
                return
...
def recalculate_outfield_stats(filtered_player, filtered_matches):
    # ... (copy from outfield_components.py) ...
    total_goals = sum(match.get("Goals", 0) for match in filtered_matches)
    total_assists = sum(match.get("Assists", 0) for match in filtered_matches)
    total_shots = sum(match.get("Shots", 0) for match in filtered_matches)
    total_shots_on_target = sum(match.get("Shots On Target", 0) for match in filtered_matches)
