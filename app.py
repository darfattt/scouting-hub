...
# Player Search page
    ...
    # Get all player data
    if not rag.data_processor.player_data:
        rag.data_processor.process_data()
    ...
    with col2:
        team_filter = st.selectbox(
            "Filter by team:",
            ["All Teams"] + sorted(list({rag.data_processor.player_data[p]["team"] for p in players}))
        )
    
    # Filter players
    filtered_players = []
    for player in players:
        if search_name.lower() in player.lower() and (team_filter == "All Teams" or rag.data_processor.player_data[player]["team"] == team_filter):
    ...
    if filtered_players:
        ...
        for player in filtered_players:
            stats = rag.data_processor.player_data[player]
            data.append({
                "Player": player,
                "Team": stats["team"],
        ...
        if selected_player:
            ...
            stats = rag.data_processor.player_data[selected_player]
...
# Player Comparison page
    ...
    players = sorted(list(rag.data_processor.player_data.keys()))
    ...
    if player1 and player2:
        # Get comparison data
        comparison = rag.compare_players(player1, player2)
