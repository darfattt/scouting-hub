...
def render_outfield_performance_analysis(data_provider, filtered_data=None, position_type="All Outfield"):
    ...
    Args:
        ...
        position_type: Type of position (Forwards, Midfielders, Defenders, All Outfield)
    """
    st.header(f"{position_type} Performance Analysis")

    # Get all player data
    if filtered_data is None:
        if hasattr(data_provider, 'data_processor') and data_provider.data_processor.player_data:
            player_data = data_provider.data_processor.player_data
        else:
            st.warning("No data processor available.")
            return
    else:
    ...
    for player in players:
        ...
        if stats.get("matches", 0) >= 5:  # Only include players with enough matches
            # Calculate derived metrics
            minutes = stats.get("minutes", 0)
            goals = stats.get("goals", 0)
            assists = stats.get("assists", 0)
            ...
            pass_accuracy = (passes_accurate / passes * 100) if passes > 0 else 0
            duel_success_rate = (duels_won / duels * 100) if duels > 0 else 0
            ...
            data.append({
                ...
                "Duels Won": duels_won,
                "Duel Success Rate": duel_success_rate
            })
    ...
    # Analysis options based on position type
    if position_type == "Forwards":
        analysis_options = [
    ...
    # Goals per 90 Distribution
    if analysis_type == "Goals per 90 Distribution":
        ...
        fig = px.histogram(
            ...
            title=f"{position_type} Goals per 90 Minutes Distribution"
        )
        ...
        goals_90_values = df["Goals/90"].astype(float).dropna()
        kde_x, kde_y = [], []

        try:
            if len(goals_90_values) > 3 and goals_90_values.std() > 0:
                kde_x = np.linspace(goals_90_values.min(), goals_90_values.max(), 100)
                from scipy import stats as scipy_stats
                kde = scipy_stats.gaussian_kde(goals_90_values)
                kde_y = kde(kde_x)
        ...
        if len(kde_x) > 0:
            fig.add_trace(go.Scatter(
                ...
                line=dict(color="#ff7f0e", width=2), name="Density"
            ))

        fig.update_layout(
            xaxis_title="Goals per 90 Minutes",
            yaxis_title="Density",
            plot_bgcolor="#F9F7F2",
            paper_bgcolor="#F9F7F2",
            height=500,
        ...
        # Show top scorers
        st.subheader(f"Top 5 {position_type} by Goals per 90")
        top_scorers = df.sort_values("Goals/90", ascending=False).head(5)
        st.dataframe(top_scorers[["Player", "Team", "Position", "Goals/90", "Goals", "Matches"]])

    # Pass Accuracy Distribution
    elif analysis_type == "Pass Accuracy Distribution":
        st.subheader("Pass Accuracy Distribution")
        ...
        fig = px.histogram(
            ...
            title=f"{position_type} Pass Accuracy Distribution"
        )

        fig.update_traces(histnorm="probability density", selector=dict(type="histogram"))

        # Add KDE curve
        pass_acc_values = df["Pass Accuracy"].astype(float).dropna()
        kde_x, kde_y = [], []

        try:
            if len(pass_acc_values) > 3 and pass_acc_values.std() > 0:
                kde_x = np.linspace(pass_acc_values.min(), pass_acc_values.max(), 100)
                from scipy import stats as scipy_stats
                kde = scipy_stats.gaussian_kde(pass_acc_values)
                kde_y = kde(kde_x)
        except Exception as e:
            st.warning(f"Could not generate KDE curve: {str(e)}")
        ...
        if len(kde_x) > 0:
            fig.add_trace(go.Scatter(
                x=kde_x, y=kde_y, mode="lines",
                line=dict(color="#d62728", width=2), name="Density"
            ))
        ...
        # Show top passers
        st.subheader(f"Top 5 {position_type} by Pass Accuracy")
        top_passers = df.sort_values("Pass Accuracy", ascending=False).head(5)
        st.dataframe(top_passers[["Player", "Team", "Position", "Pass Accuracy", "Passes", "Matches"]])

    # Goals vs Assists Analysis
    elif analysis_type == "Goals vs. Assists":
        ...
        fig = px.scatter(
            ...
            color="Position",
            hover_name="Player",
            hover_data=["Team", "Matches", "Goals", "Assists"],
            title=f"{position_type} Goals vs. Assists per 90 Minutes",
            labels={
                "Goals/90": "Goals per 90 Minutes",
        ...
        # Show balanced performers
        st.subheader("Most Balanced Attackers (Goals + Assists)")
        df["Total Contribution/90"] = df["Goals/90"] + df["Assists/90"]
        top_contributors = df.sort_values("Total Contribution/90", ascending=False).head(10)
        st.dataframe(top_contributors[["Player", "Team", "Position", "Goals/90", "Assists/90", "Total Contribution/90", "Matches"]])

    # Position-specific analyses
    elif analysis_type == "Top Scorers":
        st.subheader("Top Scorers Analysis")

        # Create composite attacking score
        df["Attacking Score"] = df["Goals/90"] * 2 + df["Assists/90"] + (df["Shots"] / df["Matches"]) * 0.1

        st.subheader("Top 10 Forwards (Overall Attacking Performance)")
        top_attackers = df.sort_values("Attacking Score", ascending=False).head(10)
        st.dataframe(top_attackers[["Player", "Team", "Goals/90", "Assists/90", "Shots", "Attacking Score", "Matches"]])
        ...
        fig = px.scatter(
            ...
            color="Player",
            hover_name="Player",
            hover_data=["Team", "Attacking Score"],
            title="Top 10 Forwards: Goals vs. Assists per 90",
            labels={"Goals/90": "Goals per 90 Minutes", "Assists/90": "Assists per 90 Minutes"}
        )
    ...
    elif analysis_type == "Top Defenders":
        st.subheader("Top Defenders Analysis")

        # Create composite defensive score
        df["Defensive Score"] = df["Duel Success Rate"] / 20 + df["Pass Accuracy"] / 20 + (df["Duels Won"] / df["Matches"]) * 0.1

        st.subheader("Top 10 Defenders (Overall Defensive Performance)")
        top_defenders = df.sort_values("Defensive Score", ascending=False).head(10)
        st.dataframe(top_defenders[["Player", "Team", "Duel Success Rate", "Pass Accuracy", "Duels Won", "Defensive Score", "Matches"]])

        # Visualize top defenders
        fig = px.scatter(
            top_defenders,
            x="Duel Success Rate",
            y="Pass Accuracy",
            ...
            color="Player",
            hover_name="Player",
            hover_data=["Team", "Defensive Score"],
            title="Top 10 Defenders: Duel Success vs. Pass Accuracy",
            labels={"Duel Success Rate": "Duel Success Rate (%)", "Pass Accuracy": "Pass Accuracy (%)"}
        )
        ...
        df["Overall Score"] = (df["Goals/90"] + df["Assists/90"]) * 2 + df["Pass Accuracy"] / 50 + df["Duel Success Rate"] / 50

        st.subheader("Top 10 Players (Overall Performance)")
        top_overall = df.sort_values("Overall Score", ascending=False).head(10)
        st.dataframe(top_overall[["Player", "Team", "Position", "Goals/90", "Assists/90", "Pass Accuracy", "Overall Score", "Matches"]])
        ...
        fig = px.scatter(
            ...
            color="Position",
            hover_name="Player",
            hover_data=["Team", "Overall Score", "Assists/90"],
            title="Top 10 Players: Pass Accuracy vs. Goals per 90",
            labels={"Pass Accuracy": "Pass Accuracy (%)", "Goals/90": "Goals per 90 Minutes"}
        )
